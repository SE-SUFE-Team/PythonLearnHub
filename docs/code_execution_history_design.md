...
通过引入 `record_type` 字段，`CodeExecution` 表能够同时存储“通用代码执行历史”和“OJ题目提交记录”两种不同业务场景的数据。
这种设计避免了为每个业务场景创建独立的表（如 `oj_submission`），简化了数据模型，降低了维护成本。
未来如果需要支持更多类型的代码记录（如代码分享、草稿等），只需扩展 `record_type` 的枚举值即可，具有良好的可扩展性。
